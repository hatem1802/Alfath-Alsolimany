...
import {
  AFTER_SAFAR,
  BADDEST_DAY,
  BAD_DAYS_IN_THE_MONTH,
  BAD_DAYS_IN_THE_YEAR_1,
  BAD_DAYS_IN_THE_YEAR_2,
  EACH_MONTHS_STATUS,
  EACH_WEEK_STATUS,
  MIXED_DAYS_NAME,
  SELECTED_DAYS,
...
function resetDateHigry(theDate = new Date()) {
  ...
  return `${hijri._date} ${Calender.months[hijri._month]} ${hijri._year}`;
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  prevDay.addEventListener("click", () => {
    ...
    getDayStatus(document.querySelector(".day-content"));
  });
...
function loadDate(latitude, longitude, selectedDate = latAndLong.dayDate) {
  ...
  function changeTheme() {
    ...
    if (timeCheckAmOrPm === "am") {
      document.getElementById("displayDay").setAttribute("data-date", theDate);
  ...
  let dayLightSecond = Math.round(
    (`.${
      parseFloat(dayLenLight / 12)
        .toFixed(2)
        .split(".")[1]
    }` *
      60) %
  ...
  let dayNightSecond = Math.round(
    (`.${
      parseFloat(dayLenNight / 12)
        .toFixed(2)
        .split(".")[1]
    }` *
      60) %
  ...
  document.querySelector("#nav-darken em").innerHTML = `${Math.floor(
    dayLenNight / 12
  )} دقيقة و ${
    dayNightSecond > 0 && dayNightSecond < 10
      ? `${dayNightSecond} ثواني`
      : `${dayNightSecond} ثانية`
  }`;
  document.querySelector("#nav-lighten em").innerHTML = `${Math.floor(
    dayLenLight / 12
  )} دقيقة و ${
    dayLightSecond > 0 && dayLightSecond < 10
      ? `${dayLightSecond} ثواني`
      : `${dayLightSecond} ثانية`
  }`;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    homePlanetHour?.hours?.push({
      start,
      end,
      week: weeks[`${theDay}light`][i],
    });
  }
...
function getDayStatus(selectedDate) {
  let date = new Date(selectedDate || latAndLong.dayDate);
  let hijriDate = date.toHijri();
  let hijriYear = hijriDate._year;
  let hijriMonth = hijriDate._month;
  let hijriDay = hijriDate._date;
  const hijri = new HijriDate(hijriYear, hijriMonth, hijriDate._date);

  let monthCount = Calender.countDayOfMonth(hijriMonth, hijriYear);

  if (Calender.leapYears?.includes(hijriYear) && hijriMonth === 12) {
    monthCount = 30;
  }
  ...
  let isBadDay1 = BAD_DAYS_IN_THE_YEAR_1?.[`${hijriMonth}-${hijriDay}`];
  let isBadDay2 = BAD_DAYS_IN_THE_YEAR_2?.[`${hijriMonth}-${hijriDay}`];
  let badDayInMonth = BAD_DAYS_IN_THE_MONTH[hijriDay];
  let isLastWed = hijriDay >= lastWeekStart && gerogenDate.getDay() === 3;
  let monthStatus = EACH_MONTHS_STATUS[monthCount];

  let currentMonth = Calender.months[hijriMonth]?.replace(/شهر/gi, "")?.trim();

  let dayType = "";
  if (SELECTED_DAYS?.[hijriDay]) {
    dayType = {
      msg: `${SELECTED_DAYS?.[hijriDay]} من شهر ${currentMonth} و  (سعد)`,
      classes: "active-good",
    };
  }
  if (MIXED_DAYS_NAME?.[hijriDay]) {
    dayType = {
      msg: `${MIXED_DAYS_NAME?.[hijriDay]} من شهر ${currentMonth} و  (ممتزج)`,
      classes: "active-normal",
    };
  }
  if (UNLUCKY_DAYS?.[hijriDay]) {
    dayType = {
      msg: `${UNLUCKY_DAYS?.[hijriDay]} من شهر ${currentMonth} و  (نحس)`,
      classes: "active-bad",
    };
  }

  if (hijriMonth === 2 && hijriDay === 6) {
    afterSafer = AFTER_SAFAR?.[new Date(gerogen).getDay()];
  }
  let content = "";

  if (isBadDay1) {
    dayType.classes = "active-bad";

    content = `
    <h4 class="${
      dayType.classes
    }">${`${SELECTED_DAYS_COLLECTION?.[hijriDay]} من شهر ${currentMonth}`}</h4>
    <ul class="d-flex flex-column align-items-start mt-2 gap-1 px-2 mb-4 list-unstyled active-day-year-bad ${
      dayType.classes
    }">
    ...
    content = `
        ...
        <ul class="d-flex flex-column align-items-start mt-2 gap-1 px-2 mb-4 list-unstyled ${
          dayType.classes
        }">
        ${afterSafer ? `<li>- ${afterSafer}</li>` : ""}
        ${`<li>- ${dayType.msg}</li>`}
        <li>- ${
          SELECTED_DAYS_COLLECTION?.[hijriDay]
        }  هو: <strong>${monthStatus}</strong></li>
        ...
        ${
          ...
          ${
            badDayInMonth
              ? `<li>- ${SELECTED_DAYS_COLLECTION?.[hijriDay]} : ${badDayInMonth}</li>`
              : ""
          }
          ${
            isBadDay2
              ? `<li>- ${isBadDay2} في ${SELECTED_DAYS_COLLECTION?.[hijriDay]} من شهر ${currentMonth} كل سنة </li>`
              : ""
          }
          ${isLastWed ? `<li>- اخر أربعاء  ${BADDEST_DAY}</li>` : ""}
          </ul>
          `;
  }
