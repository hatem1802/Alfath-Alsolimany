...
function planetTimings(latitude, longitude, selectedDate) {
  ...
  for (let i = 0; i < allHours.length; i++) {
    ...
    let MatchPmOrAm = allHours[i].start.match(/AM|PM/)?.at(0);
    const endHour = allHours[i].start.split(":").at(0);
    MatchPmOrAm = endHour > 11 ? "PM" : MatchPmOrAm;
    if (
      allHours[i].end.indexOf(MatchPmOrAm) === -1 &&
